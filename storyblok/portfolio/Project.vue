<template>
  <div class="project grid gap-5 auto-cols-fr p-5">
    ...
    <div class="project-action min-w-0 flex items-end justify-end col-start-3 col-end-4">
      <RouteComponent v-if="blok.url_project" icon-item external-link :to="blok.url_project"
        :title="`${$languageCase('link to', 'enlace por', 'link per')} ${blok.title}`">
        <template #icon>
          <IconComponent link class="project-external mr-2.5 rounded" :style="`background-color: ${$binaryControl(
            blok.background_color,
            'color',
            '#e0e0e0'
          )}; color: ${$binaryControl(blok.text_color, 'color')};`" size="w-10 h-10 p-3" />
        </template>
      </RouteComponent>
      <RouteComponent v-if="blok.url_repository" icon-item external-link :to="blok.url_repository" title="repository">
        <template #icon>
          <IconComponent git class="project-repository mr-2.5 rounded" :style="`background-color: ${$binaryControl(
            blok.background_color,
            'color',
            '#e0e0e0'
          )}; color: ${$binaryControl(blok.text_color, 'color')};`" size="w-10 h-10 p-3" />
      ...
      <IconComponent arrow :style="`background-color: ${$binaryControl(
        ...
        'color',
        '#e0e0e0'
      )}; color: ${$binaryControl(blok.text_color, 'color')};`" class="project-back rounded cursor-pointer"
        size="w-10 h-10 p-3" @click="$goBack('portfolio')" />
    </div>
    <div class="project-intro grid gap-5 auto-cols-fr col-start-1 col-end-4">
      <ModalComponent v-if="blok.image.filename"
        class="modal-project w-full row-start-1 row-end-1 xl:col-start-1 xl:col-end-3" close-mode>
        <template #activator="action">
          <div class="image-container w-full my-0 mx-auto cursor-pointer" @click="action.open()">
            <ImageComponent
              ...
              :src="blok.image.filename" :file="blok.image" :alt="blok.image?.alt" width="984" height="805"
              :style="`border-color: ${$binaryControl(blok.background_color, 'color', 'e0e0e0')}`"
              sizes="xs:380px sm:514px md:711px lg:804px xl:1240px" />
          </div>
        </template>
        <template #body>
          <ImageComponent class="image-project select-none" width="1920" height="auto"
            sizes="xs:380px sm:514px md:711px lg:804px xl:1240px 2xl:1680px" :src="blok.image.filename"
            :alt="blok.image?.alt" :file="blok.image" original />
        </template>
      </ModalComponent>
      <div :style="`background-color: ${$binaryControl(
        blok.background_color,
        'color',
        '#e0e0e0'
      )}; color: ${$binaryControl(blok.text_color, 'color')};`" :class="`intro-text markdown block self-start xl:row-start-1 xl:row-end-1 xl:col-start-3 xl:col-end-3 rounded ${setAlignText} ${!blok.remove_space ? 'p-5' : ''
    ...
    <div v-if="blok.body?.length" class="project-details grid gap-5 col-start-1 col-end-4 mt-10">
      ...
      <StoryblokComponent :is="description.component" v-for="description in blok.body" :key="description._uid"
        :blok="description" />
    </div>
  </div>
</template>
