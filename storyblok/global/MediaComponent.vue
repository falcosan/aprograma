  ...
  >
    ...
    >
      <template #activator="action">
        <ImageSet
          ...
          :width="
            blok ? (/[a-zA-Z]/.test(blok.width) ? 'auto' : blok.width) : width
          "
          :height="
            blok
              ? /[a-zA-Z]/.test(blok.height)
    ...
    <template v-else>
      <ImageSet
        ...
        "
        :height="
          blok ? (/[a-zA-Z]/.test(blok.height) ? 'auto' : blok.height) : height
        "
        :sizes="checkSizes"
      />
      <video
    ...
    <p
      ...
      :style="`color: ${
        blok && blok.title_color.color ? blok.title_color.color : false
      };`"
    ...
    checkSizes() {
      if (/100%|full|auto/.test(this.blok.width)) {
        if (/100%|full|auto/.test(this.blok.height)) {
          ...
          return `xs:${Math.round(
            this.blok.height.replace(/\D/g, "") / 1.19
          )}vw`;
        }
      } else {
        return `xs:${Math.round(
          this.blok.width.replace(/\D/g, "") / 1.15
        )}px sm:${Math.round(
          this.blok.width.replace(/\D/g, "") / 1.12
        )}px md:${Math.round(
          this.blok.width.replace(/\D/g, "") / 1.09
        )}px lg:${Math.round(
