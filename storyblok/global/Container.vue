<template>
  <div v-if="blok.body.length" ref="container" :class="`container-cover w-full ${carouselMode || sliderMode || containerMode ? 'grid' : 'parent-cover'
    }`">
    <span v-if="blok.title" :class="[
    ...
    <div :key="widthContainer" :class="`container-content h-full rounded ${blok.slider_mode === 'slider' ? 'overflow-hidden' : ''
      ...
      " :class="[
        ...
        <IconComponent v-show="(blok.slider_mode === 'slider' ||
          windowWidth < 768 ||
          !$device.isDesktop ||
          sliderMode ||
          carouselMode ||
          ...
          " previous :class="`previous-control control absolute z-20 transform rounded-full bg-opacity-70 shadow-sm cursor-pointer text-white bg-gray-500 ${blok.slider_mode === 'slider'
          ...
          :size="`${sliderMode || carouselMode ? 'w-5 h-5 p-1.5' : 'w-6 h-6 p-2'}`" @click="previous(true)" />
        <div v-show="(blok.slider_mode === 'carousel' &&
          windowWidth >= 768 &&
          $device.isDesktop &&
          !sliderMode &&
          !carouselMode &&
        ...
        <IconComponent v-show="(blok.slider_mode === 'slider' ||
          ...
          blok.row_container) &&
          !blok.hide_controllers
          " next :class="`next-control control absolute z-20 transform rounded-full bg-opacity-70 shadow-sm cursor-pointer text-white bg-gray-500 ${blok.slider_mode === 'slider'
            ? 'top-1/2 -translate-y-1/2'
            : sliderMode || carouselMode
              ? !blok.hide_dots
                ? 'bottom-3.5'
          ...
          :size="`${sliderMode || carouselMode ? 'w-5 h-5 p-1.5' : 'w-6 h-6 p-2'}`" @click="next(true)" />
        <div v-show="(blok.slider_mode === 'carousel' &&
          windowWidth >= 768 &&
          $device.isDesktop &&
          !sliderMode &&
        ...
        <div ref="sliderBox" :class="`slider-box w-full rounded ${blok.slider_mode ? 'overflow-hidden' : ''}`">
          <div v-if="blok.slider_mode === 'slider'" class="slider-container">
            <ul :style="`min-height:${$binaryControl(blok, 'height')}; transform: translateX(${-(
              (containerWidth + spaceFix) *
              sliderIndex
            )}px); gap: ${spaceFix}px;`" class="slider relative grid grid-flow-col-dense transition-transform">
              <li v-for="component in elements" :key="component._uid" ref="sliderSlide"
                :tabindex="!blok.hide_controllers ? '0' : undefined" :style="`width: ${containerWidth}px; background-color: ${$binaryControl(
                ...
                <StoryblokComponent :class="`${component.name.toLowerCase()}-component my-0 mx-auto`" :blok="component"
                  slider-mode :container-width="containerWidth" />
              </li>
            </ul>
          </div>
          <div v-else class="carousel-container">
            <TransitionGroup tag="ul" class="carousel relative grid justify-center rounded" enter-from-class="opacity-0"
              leave-to-class="opacity-0" enter-active-class="transition duration-500"
              leave-active-class="transition duration-500" :style="`min-height:${$binaryControl(blok, 'height')};`">
              <li v-for="(component, index) in elements" v-show="widthContainer ? index === currentSlide : true"
                :key="component._uid" ref="carouselSlide" :tabindex="!blok.hide_controllers ? '0' : undefined" :class="`carousel-slide slide md:relative w-full flex row-start-1 row-end-1 col-start-1 col-end-1 md:z-[1] rounded ${setHorizontalAlign} ${setVerticalAlign} ${!blok.hide_controllers ? 'outline-none' : ''
                ...
                @keydown.left.prevent="!blok.hide_controllers ? previous(true) : null">
                <StoryblokComponent :class="`${component.name.toLowerCase()}-component my-0 mx-auto`" :blok="component"
                  carousel-mode :container-width="fullWidth" />
              </li>
            </TransitionGroup>
            <div v-if="!blok.hide_dots"
              class="dot-container relative w-max max-w-1/2 flex flex-wrap justify-center z-20 my-10 mx-auto">
              <span v-for="dot in elements.length" :key="dot" :class="`dot-number_${dot} w-2.5 h-2.5 inline-block m-1.5 rounded-full shadow-inner select-none cursor-pointer transform scale-90 transition-all duration-200 ${!$device.isDesktop ? '' : 'dot-desktop'
      ...
      <div v-else class="container-box h-full overflow-hidden">
        <div :class="[
          ...
          <div v-for="component in elements" :key="component._uid" :style="`flex: ${component.row_container
            ...
            <StoryblokComponent :class="`${component.name.toLowerCase()}-component`" :blok="component" container-mode
