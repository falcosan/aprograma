  ...
  <header class="header flex justify-center">
      ...
      <div
        ...
        <TranslateComponent v-click-outside:[false]="toggleExpanded" translate-transition :class="[
          ...
          $themeColor(backgroundColor) ? 'text-white' : ''
              ...
              : '-translate-y-full'
            }`" style-translate-item="p-3 text-center" :blok="$contentByName(blok.body, 'Translate')"
          :parent-background-color="backgroundColor" @translate-list-action="toggleExpanded(false)"
    ...
    <nav v-if="layout !== 'blank'"
      ...
      <ul class="menu-wrapper wrapper-down w-full h-full max-w-sm xs:max-w-md sm:max-w-lg md:max-w-xl grid grid-cols-3">
        <li v-for="item in $contentByName(blok.body, 'Route')" :key="item._uid" class="link-menu no-underline">
