<template>
  ...
  >
    <div class="post-actions flex flex-col md:flex-row flex-wrap md:items-center md:-mt-2.5 md:-mx-2.5 md:mb-2.5">
      ...
      <div
        :class="`show-categories flex justify-self-end self-end md:self-auto row-start-2 row-end-1 mb-5 md:m-2.5 cursor-pointer rounded transition bg-gray-200 ${!$device.isDesktop ? '' : 'hover:opacity-80'
        }`"
      ...
      >
        ...
        <IconComponent
          ...
          :size="`w-3 h-3 transition ${showFilters ? 'transform -rotate-90' : 'transform rotate-90'
          }`"
        />
      </div>
    </div>
        ...
        >
          ...
          >
            ...
            <IconComponent
              close
              size="w-2.5 h-2.5"
              :class="`px-4 pointer-events-none transition ${comparedCategories.includes(filter.value) ? '' : 'transform rotate-45'
              }`"
            />
          </li>
        </ul>
    ...
    <TransitionGroup
      tag="ul"
      :class="`post-list w-full grid gap-5 auto-cols-fr auto-rows-fr ${blok.row_container || sliderMode || carouselMode || containerMode
        ? maxPosts
