...
export const useMarkdown = (init: boolean = true) => {
  const highlightCodeBlocks = () => {
    [...document.querySelectorAll('pre code')].forEach((code: Element) => {
      if (!(code instanceof HTMLElement) || code.classList.contains('syntax-code-block')) return;

      const syntax = Array.from(code.classList).find((cls) => cls.startsWith('language-'));
      if (syntax) {
        ...
        const language = hljs.getLanguage(syntax.replace('language-', ''))?.name?.toLowerCase();
  ...
  const makeImagesClickable = () => {
    ...
    document.querySelectorAll('.markdown img').forEach((image: Element) => {
      if (!(image instanceof HTMLImageElement) || image.classList.contains('image-clickable')) return;
