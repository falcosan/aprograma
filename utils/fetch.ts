...
import StoryblokClient, { type ISbResponse, type ISbStoriesParams } from 'storyblok-js-client';
...
export async function fetchStoryblok({
  ...
  lastCache = false,
  query = 'cdn/stories/',
}: { query?: string; language?: ISbStoriesParams['language'], startsWith?: ISbStoriesParams['starts_with'], lastCache?: boolean }): Promise<ISbResponse['data']> {
...
export async function fetchSitemap(): Promise<string> {
  ...
  const urls = Object.values(data.links ?? {})
    .map((link: any) => link.is_folder || link.is_startpage || !link.slug.includes('/') ? false : link.path?.substring(0, link.path.lastIndexOf('/')))
    .filter((link): link is string => !!link && !new RegExp(Data.ignore.join('|')).test(link.split('/')[0]));
