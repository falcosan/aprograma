...
export async function fetchFeed(lang) {
  const feed = new RSS({
    ...
    category: Data.rss[lang].category
  });
  const language = Data.rss[lang].language;
  const data = await fetchStoryblok(language, Data.rss.route, 'cdn/stories/', true);
  const dataFiltered = dataLang =>
    dataLang.stories.filter(filteredPost => filteredPost.name.toLowerCase() !== Data.rss.route);
  dataFiltered(data).forEach(post => {
    feed.item({
      title: post.content.title,
      image: post.content.file?.filename ?? Data.rss.image,
      guid: post.id,
...
export async function fetchSitemap() {
  ...
  const urls = Object.values(data.links)
    ...
    .filter(link => link && !new RegExp(Data.ignore.join('|')).test(link.split('/')[0]));
